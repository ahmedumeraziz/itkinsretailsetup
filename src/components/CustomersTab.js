...
function printPaymentReceipt({ customer, amountReceived, date, note, pendingBefore, remainingAfter }) {
  ...
  <style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Courier New',monospace;font-size:12px;width:302px;padding:10px 6px;color:#000;background:#fff}.sn{font-size:15px;font-weight:bold;text-align:center;margin-bottom:1px}.dv{border-top:1px dashed #000;margin:5px 0}.bi{display:flex;justify-content:space-between;font-size:10px;margin:1px 0}.row{display:flex;justify-content:space-between;margin:5px 0;font-size:12px}.big{font-size:14px;font-weight:bold}.ft{text-align:center;font-size:10px;margin-top:8px}@media print{body{margin:0}}</style>
  </head><body>
        ...
        <div style={{ maxHeight: 500, overflowY: "auto" }}>
          {filtered.length === 0 ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 140, color: "rgba(255,255,255,0.2)", gap: 8 }}>
              <div style={{ fontSize: 30 }}>👥</div><div style={{ fontSize: 12 }}>No customers found</div>
            </div>
          ) : filtered.map((c, i) => {
            const totalBilled = getTotalBilledAll(c, sales);
            const pending     = getPendingBalance(c, sales);
                ...
                {currentUser?.Role === "admin" && (
                  <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 5 }}>
                    <button className="btn" onClick={e => { e.stopPropagation(); setEditingCustomer(c); }}
                      style={{ padding: "4px 10px", background: "rgba(0,180,255,0.1)", border: "1px solid rgba(0,180,255,0.2)", color: "#00b4ff", fontSize: 11, borderRadius: 5 }}>Edit</button>
                    <button className="btn" onClick={e => handleDeleteCustomer(c, e)}
                      style={{ padding: "4px 10px", background: "rgba(255,80,80,0.1)", border: "1px solid rgba(255,80,80,0.2)", color: "#ff6b6b", fontSize: 11, borderRadius: 5 }}>Del</button>
                  </div>
                )}
              </div>
            );
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#0c1828", border: "1px solid rgba(0,180,255,0.3)", borderRadius: 14, padding: 24, width: 440, maxWidth: "95vw" }}>
            ...
            {received > 0 && (
              <>
                ...
                <div style={{ borderTop: "1px dashed rgba(255,255,255,0.12)", paddingTop: 8, display: "flex", justifyContent: "space-between" }}>
                  ...
                  <span style={{ color: remaining > 0 ? "#ff6b6b" : "#00e5a0", fontWeight: 800, fontSize: 15 }}>
                    {remaining > 0 ? `PKR ${fmt(remaining)}` : "✓ CLEAR"}
                  </span>
                </div>
              </>
            )}
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
          <div style={{ flex: 1 }}><label style={{ ...lbSt, marginBottom: 5 }}>AMOUNT (PKR)</label>
            <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="Enter amount" style={{ ...inSt, width: "100%", padding: "8px 12px", fontSize: 15 }} /></div>
          <div style={{ flex: 1 }}><label style={{ ...lbSt, marginBottom: 5 }}>DATE</label>
            <input type="date" value={date} onChange={e => setDate(e.target.value)} style={{ ...inSt, width: "100%", padding: "8px 12px" }} /></div>
        </div>
        <div style={{ marginBottom: 16 }}><label style={{ ...lbSt, marginBottom: 5 }}>NOTE</label>
