...
// ── SALES TAB ─────────────────────────────────────────────────────────────────
export function SalesTab({ sales, setSales }) {
  ...
  const reprintBill = sale => {
    ...
    printReceipt({
      ...
      grandTotal, payments: [{ type: "cash", amount: grandTotal, last4: "" }],
      change: 0, customerName: sale.CustomerName || "", customerCell: sale.CustomerCell || "", refundApplied: 0,
    });
  };
...
export function ProfitTab({ sales, items, returns }) {
  ...
  filtered.forEach(sale => {
    ...
    saleItems.forEach(si => {
      ...
      const sellPrice  = parseFloat(si.Price || 0); const costPrice = parseFloat(masterItem?.CostPrice || si.CostPrice || 0);
      const disc = parseFloat(si.Discount || 0); const qty = parseInt(si.qty) || 1;
      const revenue = (sellPrice - disc) * qty; const cost = costPrice * qty; const profit = revenue - cost;
