...
const DEMO_CUSTOMERS = [
  { Name: "Ali Khan",    CellNo: "0300-1234567", BillNo: "0115,0119" },
  { Name: "Sara Ahmed",  CellNo: "0312-9876543", BillNo: "0116"      },
  { Name: "Usman Malik", CellNo: "0321-1111111", BillNo: "0118"      },
];
const DEMO_RETURNS = [];
...
function POSScreen({ user, items, categories, billCounter, onLogout, onSaleSaved, sheetStatus, isOnline, lastSync, onRefresh, searchIndex, itemMap, sales, returns, returnCounter, onReturnSaved, onMarkReturnUsed }) {
...
const applyRefund = (refundAmt, returnNo) => {
  ...
  const canSave       = cart.length > 0 && totalReceived >= netTotal;
  ...
  const saveBill = () => {
    ...
    const bill = { billNo, date, time, cashier: user.Name, items: cart, subTotal, totalDiscount, itemDiscount, billDiscount, billDiscountPct: billDiscPct, grandTotal: netTotal, payments, change, customerName: customerInfo.Name, customerCell: customerInfo.CellNo, refundApplied };
    onSaleSaved({ BillNo: billNo, Date: date, Time: time, Cashier: user.Name, GrandTotal: netTotal, Discount: totalDiscount, FBR: 1, PaymentMethod: payments.map(p => p.type === "cash" ? "Cash" : p.type === "refund" ? `Refund(${p.origReturnNo})` : `Card(****${p.last4 || ""})`).join("+"), ItemsDetail: JSON.stringify(cart), items: cart, CustomerName: customerInfo.Name, CustomerCell: customerInfo.CellNo }, customerInfo);
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "#0a0e1a", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ width: 310, background: "rgba(255,255,255,0.012)", borderLeft: "1px solid rgba(255,255,255,0.06)", padding: 12, display: "flex", flexDirection: "column", gap: 10, overflowY: "auto" }}>
          {/* CUSTOMER */}
          <div style={{ background: "rgba(0,180,255,0.05)", border: "1px solid rgba(0,180,255,0.18)", borderRadius: 10, padding: "10px 12px" }}>
            <div style={{ color: "rgba(0,180,255,0.8)", fontSize: 10, letterSpacing: 2, fontWeight: 700, marginBottom: 8 }}>👤 CUSTOMER</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              <div><label style={{ ...lbSt, marginBottom: 3 }}>Name</label>
                <input value={ab.customerName} onChange={e => setCustName(e.target.value)} placeholder="Customer Name" tabIndex={3} style={{ ...inSt, padding: "7px 11px", fontSize: 12, border: "1px solid rgba(0,180,255,0.2)" }} /></div>
              <div><label style={{ ...lbSt, marginBottom: 3 }}>Cell No</label>
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  <input value={ab.customerCell} onChange={e => setCustCell(e.target.value)} placeholder="0300-0000000" tabIndex={4} style={{ ...inSt, padding: "7px 11px", fontSize: 12, border: "1px solid rgba(0,180,255,0.2)", flex: 1 }} />
                  {(ab.customerName !== "Unknown" || ab.customerCell !== "") && (
                    <button className="btn" onClick={() => { setCustName("Unknown"); setCustCell(""); }} tabIndex={-1} style={{ padding: "6px 9px", background: "rgba(255,80,80,0.1)", border: "1px solid rgba(255,80,80,0.2)", color: "#ff6b6b", fontSize: 11, borderRadius: 5 }}>✕</button>
                  )}
                </div>
              </div>
            </div>
          ...
          {/* PAYMENT */}
          <div style={{ color: "rgba(0,180,255,0.72)", fontSize: 10, letterSpacing: 3, fontWeight: 700 }}>PAYMENT</div>
          {payments.filter(p => p.type !== "refund").map((p, i) => {
            const realIdx = payments.indexOf(p);
            return (
              <div key={realIdx} style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: 11 }}>
                <div style={{ display: "flex", gap: 6, marginBottom: 9 }}>
                  <button className="btn" onClick={() => updPay(realIdx, "type", "cash")} tabIndex={-1} style={{ flex: 1, padding: "7px 5px", background: p.type === "cash" ? "linear-gradient(135deg,#0062ff,#00b4ff)" : "rgba(255,255,255,0.05)", border: p.type === "cash" ? "none" : "1px solid rgba(255,255,255,0.08)", color: "#fff", fontSize: 11, borderRadius: 6 }}>💵 CASH</button>
                  <button className="btn" onClick={() => updPay(realIdx, "type", "card")} tabIndex={-1} style={{ flex: 1, padding: "7px 5px", background: p.type === "card" ? "linear-gradient(135deg,#6a0dad,#9b59b6)" : "rgba(255,255,255,0.05)", border: p.type === "card" ? "none" : "1px solid rgba(255,255,255,0.08)", color: "#fff", fontSize: 11, borderRadius: 6 }}>💳 CARD</button>
                </div>
                {p.type === "card" && <input value={p.last4} onChange={e => updPay(realIdx, "last4", e.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="Last 4 digits" maxLength={4} tabIndex={5} style={{ ...inSt, marginBottom: 8, letterSpacing: 6, textAlign: "center", fontSize: 15, border: "1px solid rgba(150,100,255,0.32)" }} />}
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  <span style={{ color: "rgba(255,255,255,0.35)", fontSize: 11, whiteSpace: "nowrap" }}>PKR</span>
                  <input type="number" value={p.amount} onChange={e => updPay(realIdx, "amount", e.target.value)} placeholder="Amount" tabIndex={6} style={{ ...inSt, flex: 1, fontSize: 15 }} />
                  {payments.filter(x => x.type !== "refund").length > 1 && <button className="btn" onClick={() => delPay(realIdx)} tabIndex={-1} style={{ width: 28, height: 28, background: "rgba(255,80,80,0.1)", border: "1px solid rgba(255,80,80,0.2)", color: "#ff6b6b", borderRadius: 6, padding: 0, fontSize: 14 }}>✕</button>}
                </div>
              </div>
            );
          })}
          {payments.filter(p => p.type === "refund").map((p, i) => (
  <div key={"ref" + i} style={{ background: "rgba(255,150,0,0.07)", border: "1px solid rgba(255,150,0,0.28)", borderRadius: 10, padding: "10px 13px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
    <span style={{ color: "#ff9500", fontSize: 12 }}>↩ Refund — Return #{p.origReturnNo}</span>
              <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                <span style={{ color: "#ff9500", fontWeight: 700 }}>− PKR {fmt(p.amount)}</span>
                <button className="btn" onClick={() => delPay(payments.indexOf(p))} tabIndex={-1} style={{ width: 24, height: 24, background: "rgba(255,80,80,0.1)", border: "1px solid rgba(255,80,80,0.2)", color: "#ff6b6b", borderRadius: 5, padding: 0, fontSize: 12 }}>✕</button>
              </div>
            </div>
          ))}

          <button className="btn" onClick={addPay} tabIndex={-1} style={{ padding: 9, background: "rgba(255,255,255,0.03)", border: "1px dashed rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.38)", fontSize: 12, borderRadius: 8 }}>+ Add Payment Method</button>
          ...
          <div style={{ background: "rgba(0,180,255,0.04)", border: "1px solid rgba(0,180,255,0.13)", borderRadius: 10, padding: 13 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, fontSize: 12, color: "rgba(255,255,255,0.48)" }}><span>Total Received</span><span style={{ color: "#00e5a0" }}>PKR {fmt(totalReceived)}</span></div>
            <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 7, borderTop: "1px solid rgba(255,255,255,0.08)" }}>
              <span style={{ color: "#fff", fontWeight: 700, fontSize: 14 }}>{change >= 0 ? "CHANGE" : "REMAINING"}</span>
              <span style={{ color: change >= 0 ? "#ffd700" : "#ff6b6b", fontWeight: 800, fontSize: 18, fontFamily: "Orbitron" }}>PKR {fmt(Math.abs(change))}</span>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 7 }}>
            ...
            <button className="btn" onClick={saveBill} disabled={!canSave} tabIndex={7} style={{ flex: 2, padding: 11, background: canSave ? "linear-gradient(135deg,#00a651,#00e5a0)" : "rgba(255,255,255,0.04)", border: "none", color: canSave ? "#000" : "rgba(255,255,255,0.16)", fontSize: 12, fontWeight: 700, borderRadius: 8, letterSpacing: 1 }}>
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "#0a0e1a" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 18 }}>
        ...
        {tab === "customers"  && <CustomersTab  customers={customers} setCustomers={setCustomers} safeCallScript={safeCallScript} />}
...
// ── CUSTOMERS TAB ─────────────────────────────────────────────────────────────
function CustomersTab({ customers, setCustomers, safeCallScript }) {
  ...
  const exportCSV = () => {
    const header = "Name,CellNo\n";
    const rows = filtered.map(c => `"${(c.Name || "").replace(/"/g, '""')}","${(c.CellNo || "").replace(/"/g, '""')}"`).join("\n");
  ...
  return (
    <div>
      ...
      <div style={{ background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, overflow: "hidden" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 180px 1fr", padding: "8px 14px", background: "rgba(0,180,255,0.07)", color: "rgba(0,180,255,0.72)", fontSize: 10, letterSpacing: 2, fontWeight: 700 }}>
        ...
        <div style={{ maxHeight: 500, overflowY: "auto" }}>
          {filtered.length === 0 ? (
            ...
            <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 180px 1fr", padding: "10px 14px", borderBottom: "1px solid rgba(255,255,255,0.035)", alignItems: "center" }}>
